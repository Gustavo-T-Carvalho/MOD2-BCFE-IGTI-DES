...
citiesWithElection = cities.map(city => {
  const electionByCity = election.filter(election => {
    return election.cityId === city.id;
  });
  return { ...city, elections: electionByCity };
});

citiesWithElection = citiesWithElection.map(city => {
  let elections = city.elections
    .map(election => {
      let filteredCandidates = candidates.filter(
        candidate => candidate.id === election.candidateId
      );
      let candidateName = filteredCandidates[0].name;
  ...
  return { ...city, elections };
});

citiesWithElection.forEach(city => {
  console.log(city);
});
